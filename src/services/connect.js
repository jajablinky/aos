...
export async function dryrun({ processId, wallet, tags, data }, spinner) {
  ...
  return await connect(getInfo()).dryrun({
    process: processId,
    Owner,
    tags,
    data,
...
export function printLive() {
  keys(globalThis.alerts).map((k) => {
    if (globalThis.alerts[k].print) {
      ...
      const prompt = globalThis.alerts[k].prompt || "aos> ";
      ...
      if (typeof globalThis.setPrompt === "function") {
        globalThis.setPrompt(prompt);
      }
      process.stdout.write("\n" + prompt || "aos> ");
    }
  });
  ...
  const checkLive = async () => {
    if (!isJobRunning) {
      try {
        ...
        edges = edges.sort(
          (a, b) =>
            JSON.parse(atob(a.cursor)).ordinate -
            JSON.parse(atob(b.cursor)).ordinate,
        );
