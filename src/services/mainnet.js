...
export async function readResultMainnet({ message, process: processId }) {
  const wallet =
    typeof process.env.WALLET == "string"
...
export function printLiveMainnet() {
  ...
  keys(globalThis.alerts).map((k) => {
    if (globalThis.alerts[k] && globalThis.alerts[k].print) {
      ...
      const prompt = globalThis.alerts[k].prompt;
      if (prompt) {
        globalThis.prompt = prompt;
      }

      const delivered = emitEvent("message.received", {
        data: globalThis.alerts[k].data,
        prompt,
        source: "live",
      });

      if (prompt) {
        emitEvent("prompt.updated", { prompt });
      }
          ...
          if (currentSlot > cursor && !_processedSlots.has(currentSlot)) {
            ...
            if (!globalThis.alerts[currentSlot]) {
              globalThis.alerts[currentSlot] = {
                data:
                  resultNode.Output?.data?.output ||
                  resultNode.Output?.data ||
                  "",
                prompt:
                  resultNode.Output?.data?.prompt || resultNode.Output?.prompt,
                print: true,
              };
            }
...
export async function handleNodeTopup(jwk, insufficientBalance) {
  ...
  if (balance) {
    ...
    if (balance <= 0) {
      console.log(
        chalk.red(
          `This wallet must hold some ${PAYMENT.ticker} in order to transfer to the relay.`,
        ),
      );
      process.exit(1);
    }
    ...
    console.log(
      chalk.gray(
        "Current balance in node: " +
          chalk.green(
            `${fromDenominatedAmount(currentNodeBalance)} ${PAYMENT.ticker}\n`,
          ),
      ),
    );
  } catch (e) {
  ...
  if (insufficientBalance) {
    const answer = await ask(
      chalk.gray("Insufficient funds. Would you like to top up? (Y/N): "),
    );
            ...
            console.log(
              chalk.gray(
                "Updated balance in node: " +
                  chalk.green(
                    `${fromDenominatedAmount(updatedNodeBalance)} ${PAYMENT.ticker}`,
                  ),
              ),
            );
          }
