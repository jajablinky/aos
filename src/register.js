...
const promptSelect = async ({ message, choices, name, promptUser }) => {
  if (promptUser) {
    return promptUser({ message, choices, name });
  }
  ...
  return prompts({
    ...
    .then((r) => r[name])
    .catch(() => Promise.reject({ ok: false, error: "No selection made" }));
};

const promptModule = (results, promptUser) => {
  const choices = results.map((res, i) => {
    const format =
      res.node.tags.find((t) => t.name === "Module-Format")?.value ??
      "Unknown Format";
    ...
    try {
      ...
      if (cachedTx) {
        ...
        const variantTag = tags.find(
          (tag) => tag.name.toLowerCase() === "variant",
        );
...
async function handleExistingProcess(results) {
  if (results.length === 1) {
    ...
    const variant = results[0].node.tags.find(
      (t) => t.name.toLowerCase() === "variant",
    )?.value;
  ...
  const processes = results.map((r, i) => {
    const version = r.node.tags.find(
      (t) => t.name.toLowerCase() === "aos-version",
    )?.value;
    const variant = r.node.tags.find(
      (t) => t.name.toLowerCase() === "variant",
    )?.value;
  ...
  const selectedProcessId = await promptSelect({
    name: "process",
  ...
  const selectedProcess = processes.find((p) => p.value === selectedProcessId);
  return { id: selectedProcessId, variant: selectedProcess?.variant };
}
...
async function findModule(services, moduleArg) {
  ...
  try {
    const gqlResult = await services.gql(findAoModuleByName(), {
      name: moduleArg,
    });
    const edges = utils.path(["data", "transactions", "edges"], gqlResult);
    ...
    const moduleId = await promptModule(edges, services.promptUser);
    return moduleId;
  } catch (error) {
